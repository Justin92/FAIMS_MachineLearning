...
s<-read.csv("P:/JGM_FAIMS_CVprediction/JMM_new_data/Peptide_area_q_idotp_20200108.csv", stringsAsFactors = F)
...
cvcols<-list()
for(x in cv_values){
  cvcols[[as.character(x)]]<-grep(colnames(s_area), pattern=paste("X",as.character(x), sep=""))
}
...
colnames(output4machinelearning)[23]<-"maxCV"
head(output4machinelearning, 10)

write.table(output4machinelearning, file="P:/JGM_FAIMS_CVprediction/JMM_new_data/NEW_JMMdata_maxCVvalues.txt",
